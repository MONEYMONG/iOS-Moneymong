...
final class UploadReceiptUseCaseTests: XCTestCase {
  var sut: UploadReceiptUseCase!
  ...
  override func setUpWithError() throws {
    ...
    sut = UploadReceiptUseCase(ledgerRepo: ledgerRepo)
  }
  ...
  func test_execute_호출_시_에러가_발생하지_않는다면_Network_요청이_발생한다() async {
    ...
    do {
      // Act
      try await sut.execute(ledgerID: 0, receiptImageUrls: [""])
