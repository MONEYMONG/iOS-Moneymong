...
public final class AgencyRepository: AgencyRepositoryInterface {
  ...
  public func create(name: String, description: String) async throws {
    let targetType = AgencyAPI.create(param: .init(name: name, description: description, agencyType: "STUDENT_COUNCIL"))
    let dto = try await networkManager.request(target: targetType, of: AgencyIDResponseDTO.self)
  }
